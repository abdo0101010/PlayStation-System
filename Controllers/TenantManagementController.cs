    ...
    public class TenantManagementController : Controller
    {
            ...
            public async Task<IActionResult> Create(CreateTenantViewModel model)
            {
                ...
                var adminUser = new ApplicationUser
                {
                    ...
                    TenantId = newTenant.Id, // ربط المستخدم بالمحل
                    IsActive = true,
                    EmailConfirmed = true
                };
                ...
                // 5. حفظ المحل في قاعدة البيانات
                await _context.Tenants.AddAsync(newTenant);
                await _context.SaveChangesAsync();

                TempData["SuccessMessage"] = $"تم إنشاء المحل '{newTenant.Name}' وحساب الأدمن الخاص به بنجاح.";
